...
void read_ws() {
	...
	cout << s_mmf(open_mmf("mmftest_pchr")) << endl;
...
void write_ws() {
	...
	mmf::writer::l mmfWriter("mmftest_pchr", "unused..");
	mmfWriter.writ_s(u8"안녕하세요 from c++..!");
...
void read_pip() {
	...
	if (hPipe == INVALID_HANDLE_VALUE) co_s(u8"handle error!");
...
void write_pip() {
	...
	i::t size = yHdr_s(data, "hello python..! from c++");
...
void pip_ws() {
	...
	i::t size = yHdr_s(data, u8"안녕하세요 파이썬..! from c++");
	activate_cpPipe(hPipe, "\\\\.\\pipe\\Foo", 1024);
...
int main()
{
	_wsetlocale(LC_ALL, L"ko_KR.UTF-8");      //지역화 설정을 전역적으로 적용
	...
	s::v   vsMenu = { u8"mmf 바이트 스트링 쓰기",u8"mmf 바이트 스트링 읽기",u8"mmf 유니코드 스트링 쓰기",u8"mmf 유니코드 스트링 읽기",u8"pipe 바이트 스트링 쓰기", u8"pipe 바이트 스트링 읽기", u8"pipe 바이트 유니코드 쓰기", u8"pipe 바이트 유니코드 읽기" };
